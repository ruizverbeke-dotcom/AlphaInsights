...
• Phase 6.9+
    ...
    - `/logs/insights` for summary analytics over optimizer logs.
...
# Optional: pure insights helper (Phase 6.9+)
try:
    from analytics.log_insights import compute_optimizer_insights
except Exception as e:  # noqa: BLE001
    compute_optimizer_insights = None  # type: ignore[assignment]
    print(f"[Backend] ℹ️ analytics.log_insights not available; /logs/insights will be limited. Reason: {e}")
...
# --------------------------------------------------------------------------- #
# /logs/insights — Summary analytics over optimizer_logs (Phase 6.9+)
# --------------------------------------------------------------------------- #

@app.get("/logs/insights")
async def logs_insights(
    limit: int = Query(
        500,
        ge=1,
        le=5000,
        description="Number of recent logs to include in the insights summary.",
    ),
):
    """
    Summarize optimizer history (for dashboards & agents).

    Returns (example shape):
    {
      "source": { "count": 42, "limit": 500 },
      "insights": {
        "total_runs": 42,
        "runs_by_endpoint": {"sharpe": 30, "cvar": 12},
        "last_run_at": "...",
        "last_run_by_endpoint": {...},
        "sharpe_summary": { ...basic stats... },
        "cvar_summary": { ...basic stats... }
      }
    }
    """
    # Respect existing safety contracts:
    if not SUPABASE_ENABLED or fetch_recent is None:
        raise HTTPException(status_code=503, detail="Supabase logging is disabled.")

    if compute_optimizer_insights is None:
        raise HTTPException(
            status_code=501,
            detail="analytics.log_insights is not available; insights endpoint not enabled.",
        )

    try:
        logs = fetch_recent("optimizer_logs", limit=limit, debug=False)  # type: ignore[arg-type]
        insights = compute_optimizer_insights(logs)
        return {
            "source": {
                "count": len(logs),
                "limit": limit,
...
async def status_summary():
    ...
    Now also reports whether insights are wired and which core features are active.
    """
    return {
